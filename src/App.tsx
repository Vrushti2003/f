...
export default function App() {
  ...
  const handleSubmitCode = async () => {
    ...
    try {
      ...
      if (totalTests === 0) {
        setStdout((prev) => prev + `Validating C program compilation and execution... `);
        const res = await runCCode(currentCode, currentStdin || '', 3000);
        if (res.error) {
          setStdout((prev) => prev + `[FAILED - COMPILATION / RUNTIME ERROR]\n`);
          failureDetails = `Compilation / Runtime Error:\n${res.error}`;
        } else {
          setStdout((prev) => prev + `[PASSED - COMPILED & EXECUTED CLEANLY]\n`);
          passedCount = 1;
        }
      } else {
          ...
          <div className="border-t border-neutral-800 bg-neutral-900/60 p-3 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-neutral-500 font-mono">
                {compState.activeRoundId === 'round2'
                  ? 'Debugging Round • Code Runner Disabled'
